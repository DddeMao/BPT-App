...
const UI = {
  ...
  renderSongs(songs) {
    ...
    if (!songs || songs.length === 0) { container.innerHTML = '<div style="color:#888;text-align:center;padding:40px;">Треки не найдены</div>'; return; }
    ...
    container.innerHTML = songs.map(song => {
      ...
      let scoreHtml = '';
      if (hasUserRating) {
        scoreHtml = `<div style="display:flex;align-items:center;gap:8px;margin:4px 0;">${this.renderMiniCircles(userRating.scores)}<span style="font-size:0.85rem;font-weight:600;">${userRating.total} ★</span></div>`;
        if (avg !== null) scoreHtml += `<div style="font-size:0.75rem;color:var(--text-secondary);margin-top:2px;">Средний: ${avg} ★ (${song.ratings.length})</div>`;
      } else {
        scoreHtml = `<div style="color:#888;font-style:italic;font-size:0.85rem;margin:4px 0;">Вы не оценили</div>`;
        if (avg !== null) scoreHtml += `<div style="display:flex;align-items:center;gap:8px;margin-top:4px;">${this.renderMiniCircles(CONFIG.CRITERIA.map((_,i) => {const vals=song.ratings.map(r=>r.scores[i]).filter(s=>s!==undefined);return vals.length?Math.round(vals.reduce((a,b)=>a+b,0)/vals.length):0;}))}<span style="font-size:0.75rem;color:var(--text-secondary);">${avg} ★ (${song.ratings.length})</span></div>`;
      }
      ...
      return `<div class="card" data-id="${song.id}"><div class="card-top">${coverUrl ? `<img class="card-cover" src="${coverUrl}" alt="cover">` : '<div class="card-cover"></div>'}<div class="card-info"><div class="card-title">${this.escapeHtml(song.title)}</div><div class="card-artist">${this.escapeHtml(song.artist)}</div><div class="card-meta">${albumLine} ${dateLine}</div></div></div>${scoreHtml}<div class="card-actions"><div class="card-actions-left"><button class="play-btn" data-id="${song.id}"><svg width="16" height="16" viewBox="0 0 24 24" fill="currentColor"><path d="M8 5v14l11-7z"/></svg></button></div><div class="card-actions-right"><button class="rate-btn" data-id="${song.id}"><svg width="16" height="16" viewBox="0 0 24 24" fill="currentColor">${hasUserRating ? '<path d="M3 17.25V21h3.75L17.81 9.94l-3.75-3.75L3 17.25zM20.71 7.04c.39-.39.39-1.02 0-1.41l-2.34-2.34c-.39-.39-1.02-.39-1.41 0l-1.83 1.83 3.75 3.75 1.83-1.83z"/>' : '<path d="M12 17.27L18.18 21l-1.64-7.03L22 9.24l-7.19-.61L12 2 9.19 8.63 2 9.24l5.46 4.73L5.82 21z"/>'}</svg></button><button class="comment-btn ${commentClass}" data-id="${song.id}"><svg width="16" height="16" viewBox="0 0 24 24" fill="currentColor"><path d="M21.99 4c0-1.1-.89-2-1.99-2H4c-1.1 0-2 .9-2 2v12c0 1.1.9 2 2 2h14l4 4-.01-18zM18 14H6v-2h12v2zm0-3H6V9h12v2zm0-3H6V6h12v2z"/></svg>${commentCount ? `<span class="comment-count">${commentCount}</span>` : ''}</button><button class="favorite-btn ${isFavorite ? 'is-favorite' : ''}" data-id="${song.id}"><svg width="16" height="16" viewBox="0 0 24 24" fill="${isFavorite ? 'currentColor' : 'none'}" stroke="currentColor" stroke-width="2"><path d="M12 21.35l-1.45-1.32C5.4 15.36 2 12.28 2 8.5 2 5.42 4.42 3 7.5 3c1.74 0 3.41.81 4.5 2.09C13.09 3.81 14.76 3 16.5 3 19.58 3 22 5.42 22 8.5c0 3.78-3.4 6.86-8.55 11.54L12 21.35z"/></svg></button>${canDelete ? `<button class="delete-btn" data-id="${song.id}"><svg width="16" height="16" viewBox="0 0 24 24" fill="currentColor"><path d="M6 19c0 1.1.9 2 2 2h8c1.1 0 2-.9 2-2V7H6v12zM19 4h-3.5l-1-1h-5l-1 1H5v2h14V4z"/></svg></button>` : ''}</div></div></div>`;
    }).join('');
    container.querySelectorAll('.play-btn').forEach(btn => btn.addEventListener('click', (e) => { const id = e.currentTarget.closest('.card')?.dataset.id; if (id) Player.play(id); }));
    container.querySelectorAll('.rate-btn').forEach(btn => btn.addEventListener('click', (e) => { const id = e.currentTarget.closest('.card')?.dataset.id; if (id) this.openTrackView(id, 'ratings'); }));
    container.querySelectorAll('.comment-btn').forEach(btn => btn.addEventListener('click', (e) => { const id = e.currentTarget.closest('.card')?.dataset.id; if (id) this.openTrackView(id, 'comments'); }));
    container.querySelectorAll('.favorite-btn').forEach(btn => btn.addEventListener('click', (e) => { e.stopPropagation(); const id = e.currentTarget.closest('.card')?.dataset.id; if (id) this.toggleFavorite(id); }));
    container.querySelectorAll('.delete-btn').forEach(btn => btn.addEventListener('click', (e) => { const id = e.currentTarget.closest('.card')?.dataset.id; if (id && confirm('Удалить этот трек и все его данные?')) { DB.delete(CONFIG.STORE_SONGS, id).then(() => { App.refreshAll(); Sync.onDataChanged(); }); } }));
    container.querySelectorAll('.card').forEach(card => card.addEventListener('click', (e) => { if (e.target.closest('button')) return; const id = card.dataset.id; if (id) this.openTrackView(id, 'ratings'); }));
  },
